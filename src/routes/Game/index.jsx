...
const RadialMenu = ({ upgrades, upgrade, isOpen, toggleMenu }) => (
  <div className="radial-menu">
    <input type="checkbox" id="toggle" checked={isOpen} readOnly />
    <label htmlFor="toggle" onClick={toggleMenu}>
      ...
      {upgrades.map(({ name, label, priceId }, index) => (
        <div
          ...
          onClick={(e) => {
            e.stopPropagation(); // Evita o fechamento do menu
            upgrade(label, priceId);
          }}
          style={{
            transform: `rotate(${index * (360 / upgrades.length)}deg) translate(200px) rotate(-${index * (360 / upgrades.length)}deg)`,
            opacity: isOpen ? 1 : 0,
          }}
        >
          <p>{name}</p>
...
const Game = () => {
  ...
  const upgrade = (label, priceId) => {
    console.log(`Upgrade: ${label}, Price ID: ${priceId}`);
    // Implementar lógica de upgrade
  };

  return (
    <>
      <img src={BannerGame} alt="" />
      <MainGame>
        <div className="game-container">
          ...
          <div className="controls">
            {!isGameStarted && (
              <div className="wrapper" onClick={handleStartGame}>
                <a className="cta" href="#">
                  <span>START</span>
                  <span>
                    <svg width="50px" height="50px" viewBox="0 0 66 43" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink">
            ...
            {isGameStarted && (
              <RadialMenu upgrades={upgrades} upgrade={upgrade} isOpen={isOpen} toggleMenu={toggleMenu} />
            )}
          </div>
        </div>
      </MainGame>
